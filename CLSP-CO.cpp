...
/*input boat's info*/
int io_boats(string DataFileName) {
	ifstream file;
	int LINES;
	...
	if (file.fail()) {
		...
		LINES = CountItemNumbers(DataFileName);
...
/*input route's info*/
int io_routes(string DataFileName) {
	ifstream file;
	int LINES;
	file.open(DataFileName, ios::in);
	if (file.fail()) {
...
int findMinIdleBoat(int Rindex, vector<vector<int> >& routeList) {
	...
	if (routeList[H].size() == 0) {
		...
		//cout << "Ships in idle route are: " << endl;
...
int findMinUsedBoat(int Rindex, vector<vector<int> >& routeList) {
	...
	if (routeList[Rindex - 1].size() == 0) {
		...
		//cout << "Ships in route " << Rindex << " are: " << endl;
...
void initTL(int** swapTL, int** insertTL, int tl) {
	for (int i = 0; i < J; i++) {
		for (int j = 0; j < J; j++) {
			if (i == j) {
				swapTL[i][j] = 0;
...
void outputRoutes(vector<vector<int> >& routeList) {
	for (int h = 0; h < routeList.size() - 1; h++) {
		cout << "Ships in route " << R[h].Rh << " are: " << endl;
		for (int j = 0; j < routeList[h].size(); j++) {
			cout << routeList[h][j] << "\t";
		}
		cout << endl;
...
/*output boats' speed*/
void outputShipspeed(vector<vector<int> >& routeList) {
	for (int h = 0; h < routeList.size() - 1; h++) {
...
/*update port costs*/
int updateCP(vector<vector<int> >& routeList, int** N) {
	int* rf = new int[J];
	int i = 0;
	int sum = 0;
	for (int h = 0; h < routeList.size() - 1; h++) {
...
void PTswap(vector<vector<int> >& routeList, int** swapTL, int tl, double bestSoFar, int** N, int** Q, double* Nh) {
	int s = 0;
	int r;/*random number*/
	int Bindex1, Bindex2;
		...
		if (routeList[h].size() != 0) {
			for (int j = 0; j < routeList[h].size() - 1; j++) {
				for (int k = j + 1; k < routeList[h].size(); k++) {
					swapTL[routeList[h][j] - 1][routeList[h][k] - 1] = 0;
					swapTL[routeList[h][k] - 1][routeList[h][j] - 1] = 0;
				}
			}
		}
		else {
	...
	do {
		...
		if (swapTL[Bindex1 - 1][Bindex2 - 1] == tl) {
		...
		Rindex1 = 0;
		Rindex2 = 0;
	} while ((currentSubject >= bestSoFar) && (swapTL[Bindex1 - 1][Bindex2 - 1] != tl));

	swapTL[Bindex1 - 1][Bindex2 - 1] = 0;
	swapTL[Bindex2 - 1][Bindex1 - 1] = 0;

	for (int i = 0; i < J; i++) {
		for (int j = 0; j < J; j++) {
			if (swapTL[i][j] < tl && i != j) {
				swapTL[i][j]++;
			}
		}
	}
...
/*Regular Tabu Search*/
void RTswap(vector<vector<int> >& routeList, int** swapTL, int tl, double bestSoFar, int** N, int** Q, double* Nh) {
	int s = 0;
	int r = 5;
	int Bindex1, Bindex2, Bindex0;/*swapship1,swapship2,tempship*/
	int Rindex1, Rindex2, Rindex0;/*swaproute1,swaproute2,temproute*/
	int index;
	double currentSubject = 0;
	double minSubject = 0;
...
/*insert*/
void insert(vector<vector<int> >& routeList, int Bindex, int Rindex) {
	int rindex;
	vector<int> ::iterator iter;
	int dis;
...
/*regular tabu insert*/
void RTinsert(vector<vector<int> >& routeList, int** insertTL, double bestSoFar, int** Q, int** N, int tl, int k) {
	//cout << endl;
	//cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Choosing a boat inserted to another route (RTI)......" << endl;
	int Bindex, Rindex;
	int Rindex0, Rindex2;/*0 for underload and 2 for overload*/
	int s = 0;/*the sum of table*/
	int r;/*random number*/
	int index;
	double currentSubject = 0, minSubject = 0;

	/*initial choosing table*/
	...
	do {
		...
		if (insertTL[Bindex - 1][Rindex - 1] == tl) {
			break;
		}
		else {
			index = findRoute(routeList, Bindex);
			insert(routeList, Bindex, Rindex);
			currentSubject = calSubject(routeList, N);
			if (currentSubject < bestSoFar) {
				//cout << "Find a new lower bound, lift the ban in advance." << endl;
				break;
				...
				insert(routeList, Bindex, index);
				//cout << "This operation is banned now." << endl;
				//cout << "Insert ship back: " << Bindex << "\t" << Rindex << endl;
				//cout << "Rechoosing......" << endl;
			}
		}

	} while ((currentSubject >= bestSoFar) && (insertTL[Bindex - 1][Rindex - 1] != tl));
...
/*probabilistic tabu insert*/
void PTinsert(vector<vector<int> >& routeList, int** insertTL, double bestSoFar, int** Q, int** N, int tl, int k) {
	//cout << endl;
	//cout << "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Choosing a boat inserted to another route (PTI)......" << endl;
	...
	int a = 0, b = 0;/*the number of underload route and overload routes*/
	for (int h = 0; h < routeList.size() - 1; h++) {
		if (R[h].Sh == 0) {
			a += 1;
			Rindex0 = R[h].Rh;
...
void Tspeed(vector<vector<int> >& routeList, int** Q, int** N, double sd, double su, int tl, double* Nh) {
	...
	for (int h = 0; h < routeList.size() - 1; h++) {
		if (R[h].Sh == 0) {
			//cout << "Ships on route " << R[h].Rh << " should speed up." << endl;
			c = 0;
			for (int j = 0; j < routeList[h].size(); j++) {
			...
			//cout << "Ships on route " << R[h].Rh << " can decelerate a little bit." << endl;
			updateN(N);
			for (int j = 0; j < routeList[h].size(); j++) {
			...
			//cout << "Ships on route " << R[h].Rh << " should decelerate." << endl;
			c = 0;
			for (int j = 0; j < routeList[h].size(); j++) {
...
int main() {
	...
	J = io_boats("ship1.txt");
	H = io_routes("route1.txt");
	...
	int p = 0, P = 150;/*search times,iterations*/
	...
	/*tabu list*/
	int** swapTL = new int* [J];/*create rows*/
	for (int j = 0; j < J; j++) {
		swapTL[j] = new int[J];/*create columns*/
	}

	int** insertTL = new int* [J];/*create rows*/
	for (int j = 0; j < J; j++) {
		insertTL[j] = new int[H + 1];/*create columns*/
	}

	/*---------------------------------construct initial solution---------------------------------*/
	initRouteLists(routeList);/*initial route list (including idle route)*/
	initTL(swapTL, insertTL, tl);/*initial TS swap table*/
	...
	while (p < P) {
		...
		PTswap(routeList, swapTL, tl, bestSoFar, N, Q, Nh);
		//RTswap(routeList, swapTL, tl, bestSoFar, N, Q, Nh);
		identifyRouteState(routeList, Q, N, Nh);
		PTinsert(routeList, insertTL, bestSoFar, Q, N, tl, k);
		//RTinsert(routeList, insertTL, bestSoFar, Q, N, tl, k);
		identifyRouteState(routeList, Q, N, Nh);
	...
	/*---------------------------------Output results---------------------------------*/
	cout << endl;
	cout << P << " Iterations Completed!" << endl;
	cout << "********************************************************************************" << endl;
